...
namespace coen79_lab4
{
    ...
    public:
        ...
        // Constant member functions for the keyed_bag class
        bool has_key(const key_type &key) const;
        keyed_bag::value_type get(const key_type &key) const;
        keyed_bag::size_type size() const;
        keyed_bag::size_type count(const value_type& target) const;
        bool hasDuplicateKey(const keyed_bag& otherBag) const;
    
    private:
    // Private member variables
        keyed_bag::value_type data[CAPACITY];
        keyed_bag::key_type keys[CAPACITY];
        keyed_bag::size_type length;
